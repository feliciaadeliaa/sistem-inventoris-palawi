<div class="relative" x-data="{
    ...
    <div class="flex items-center text-gray-700 dark:text-gray-400">
        ...
        <!-- Foto Profil: langsung ke halaman edit profil -->
        <a href="{{ route('profile.edit') }}" class="mr-3 overflow-hidden rounded-full h-11 w-11 block">
            <img
                src="{{ auth()->user()->avatar ? asset('storage/' . auth()->user()->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=random' }}"
                alt="{{ auth()->user()->name }}"
            />
        </a>

        <!-- Nama + Chevron: tetap toggle dropdown -->
        <button
        ...
        >
            ...
            <!-- Chevron Icon -->
            <svg
                class="stroke-gray-500 dark:stroke-gray-400 transition-transform duration-200"
